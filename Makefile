...
chat:
	docker run --rm -it --env-file .env -v "$$(pwd)/.cache:/app/.cache" -e HF_HOME=/app/.cache/huggingface -v "$$(pwd)/data:/app/data" -v "$$(pwd)/outputs:/app/outputs" -v "$$(pwd)/reports:/app/reports" llm-finetune-cpu chat --config configs/default.yaml --adapter outputs/final_adapter
